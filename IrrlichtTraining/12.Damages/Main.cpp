...
int main()
{
	...
	for (int i = 0; i < 7; i++)
	{
		std::string filename = "enemy" + std::to_string(i) + ".png";
		enemyTextures[i] = driver->getTexture(filename.c_str());
	}

	// Will create an enemy
	auto enemy = addEntity<Enemy>(3, 4);
	enemy->setTextures(enemyTextures);
